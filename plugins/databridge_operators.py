...
class DataBridgeToS3Operator(AWSBatchOperator):
    ...
    template_fields = ('table_schema', 'table_name')
    ui_color = '#ededed'
    ...
    super(DataBridgeToS3Operator, self).__init__(
        job_name='db_to_s3_{}_{}'.format(self.table_schema, self.table_name),
        ...
        overrides={
            'command': [
                ...
                'db_table_schema={}'.format(self.table_schema), 
                'db_table_name={}'.format(self.table_name), 
                's3_bucket=citygeo-airflow-databridge2',
            ],
        },
        task_id='db_to_s3_{}_{}'.format(self.table_schema, self.table_name),
        *args, **kwargs)
...
class S3ToDataBridge2Operator(AWSBatchOperator):
    ...
    super(S3ToDataBridge2Operator, self).__init__(
        job_name='s3_to_databridge2_{}_{}'.format(self.table_schema, self.table_name),
        job_definition='test_extract_and_load_to_databridge',
        job_queue='databridge-airflow',
        region_name='us-east-1',
        overrides={
        ...
        task_id='s3_to_databridge2_{}_{}'.format(self.table_schema, self.table_name),
        *args, **kwargs)
...
class S3ToCartoOperator(AWSBatchOperator):
    ...
    super(S3ToCartoOperator, self).__init__(
        job_name='s3_to_carto_{}_{}'.format(self.table_schema, self.table_name),
        job_definition='test_extract_and_load_to_databridge',
        job_queue='databridge-airflow',
        region_name='us-east-1',
        overrides={
            'command': [
                ...
                's3_bucket=citygeo-airflow-databridge2',
                'json_schema_file={}__{}.json'.format(self.table_schema, self.table_name),
            ],
        },
        task_id='s3_to_carto_{}_{}'.format(self.table_schema, self.table_name),
        *args, **kwargs)