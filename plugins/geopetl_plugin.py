...
class GeopetlReadOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        # check for null bytes ('\0'), and if exists replace with null string (''):
...
class GeopetlWriteOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        if self.hook.conn_type == 'postgres':
            ...
            table_fields = [f[0] for f in cur.fetchall()]
            table_fields_in_csv = list(set(header) & set(table_fields))
            rows = rows.cut(table_fields_in_csv)
            if self.append:
                rows.appendpostgis(self.conn, self.db_table_name)
            else:
         ...
         " NLS_TIMESTAMP_TZ_FORMAT = 'YYYY-MM-DD HH24:MI:SS.FF TZH:TZM'")
            if self.append:
                ...
                etl.fromcsv(self.csv_path, encoding='utf-8').appendoraclesde(self.conn, self.db_table_name, fail_on_empty=self.fail_on_empty)
            else:
                print('Truncating and appending rows...')
                etl.fromcsv(self.csv_path, encoding='utf-8').tooraclesde(self.conn, self.db_table_name, fail_on_empty=self.fail_on_empty)
        logging.info("Done!")


class GeopetlUpsertOperator(BaseOperator):
    ...
    def __init__(self,
        ...
        super(GeopetlUpsertOperator, self).__init__(*args, **kwargs)
        self.db_conn_id = db_conn_id
        self.db_table_name = db_table_name
...
class GeopetlPlugin(AirflowPlugin):
    name = "geopetl_plugin"
    operators = [GeopetlReadOperator, GeopetlWriteOperator, GeopetlUpsertOperator]
    hooks = [GeopetlHook]
