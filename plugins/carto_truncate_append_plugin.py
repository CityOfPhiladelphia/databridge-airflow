...
class CartoHook(BaseHook):
    ...
    def get_conn(self):
        ...
        if not (account and api_key):
            creds = re.match(carto_connection_string_regex,
                             connection_string).groups()
            account = creds[0]
            api_key = creds[1]
        auth_client = APIKeyAuthClient(
            api_key=api_key, base_url=USR_BASE_URL.format(user=account))
        sql = SQLClient(auth_client)

        return sql, account, api_key, USR_BASE_URL


class CartoTruncateAppendOperator(BaseOperator):
    ...
    def __init__(self,
                 ...
                 db_schema_json,
                 *args, **kwargs):
        super(CartoTruncateAppendOperator, self).__init__(*args, **kwargs)
        self.db_conn_id = db_conn_id
    ...
    def format_schema(self):
        ...
        with open(self.db_schema_json) as json_file:
            ...
            for i, scheme in enumerate(schema):
                scheme_type = mapping.get(
                    scheme['type'].lower(), scheme['type'])
                if scheme_type == 'geometry':
                    scheme_srid = scheme.get('srid', '')
                    scheme_geometry_type = geom_type_mapping.get(
                        scheme.get('geometry_type', '').lower(), '')
                    if scheme_srid and scheme_geometry_type:
                        scheme_type = '''geometry ({}, {}) '''.format(
                            scheme_geometry_type, scheme_srid)
                    else:
                        logging.error(
                            'srid and geometry_type must be provided with geometry field...')
                        raise
    ...
    def create_table(self):
        self.format_schema()
        stmt = ''' CREATE TABLE {table_name} ({schema})'''.format(
            table_name=self.temp_table_name, schema=self.schema_fmt)
        print(stmt)
        self.carto_sql_call(stmt)
        check_table_sql = "SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = '{}');"
        response = self.carto_sql_call(
            check_table_sql.format(self.temp_table_name))
    ...
    def write(self):
        ...
        if self.geom_field and self.geom_srid:
            rows = rows.convert(self.geom_field, lambda c: 'SRID={srid};{geom}'.format(
                srid=self.geom_srid, geom=c) if c else '')
        ...
        for i, field in enumerate(header):
            if i < num_fields - 1:
                ...
                str_header += field
        if self.geom_field:
            # carto_header = list(map(lambda x: x.replace(self.geom_field, 'the_geom'), header))
            self.carto_header = str_header.replace(self.geom_field, 'the_geom') + ',the_geom_webmercator'
            print(self.carto_header)
        else:
            self.carto_header = str_header
            
        q = "COPY {table_name} ({header}) FROM STDIN WITH (FORMAT csv, HEADER true)".format(
            table_name=self.temp_table_name, header=str_header)
        url = self.USR_BASE_URL.format(
            user=self.account) + 'api/v2/sql/copyfrom'
        with open(write_file, 'rb') as f:
            r = requests.post(
                url, params={'api_key': self.api_key, 'q': q}, data=f, stream=True)
    ...
    def verify_count(self):
        data = self.carto_sql_call(
            'SELECT count(*) FROM "{}";'.format(self.temp_table_name))
        num_rows_in_table = data['rows'][0]['count']
        # for now until inserts/upserts are implemented
        num_rows_inserted = num_rows_in_table
    ...
    def cartodbfytable(self):
        logging.info(
            '{} - cdb_cartodbfytable\'ing table'.format(self.temp_table_name))
        self.carto_sql_call("select cdb_cartodbfytable('{}', '{}');".format(
            self.account, self.temp_table_name))

    def vacuum_analyze(self):
        logging.info(
            '{} - vacuum analyzing table'.format(self.temp_table_name))
        self.carto_sql_call(
            'VACUUM ANALYZE "{}";'.format(self.temp_table_name))

    def truncate_append(self):
        stmt = 'BEGIN;' + \
            'TRUNCATE TABLE "{}";'.format(self.db_table_name) + \
            'INSERT INTO "{prod_table}" ({str_header}) SELECT {str_header} FROM "{temp_table}";'.format(prod_table=self.db_table_name, temp_table=self.temp_table_name, str_header=self.carto_header) + \
            'COMMIT;'
        self.carto_sql_call(stmt)

    def cleanup(self):
        print("Attempting to drop any temporary tables: {}".format(
            self.temp_table_name))
        stmt = '''DROP TABLE if exists {} cascade'''.format(
            self.temp_table_name)
        self.carto_sql_call(stmt)
    ...
    def execute(self, context):
     ...
     # Create temp table:
        try:
            ...
            #     self.create_indexes()
            # Truncate append prod table:
            print("truncating and appending prod table...")
            self.truncate_append()
            # Vacuum analyze:
...
class CartoTruncateAppendPlugin(AirflowPlugin):
    name = "carto_truncate_append_plugin"
    operators = [CartoTruncateAppendOperator]
    hooks = [CartoHook]
