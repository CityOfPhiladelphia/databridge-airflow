...
#pipeline = DAG('etl_cama_geopetl_dev_v1', schedule_interval='0 23 * * 0,1,2,3,4,5', default_args=default_args)
pipeline = DAG('etl_cama_geopetl_dev_v1', schedule_interval=None, default_args=default_args)
...
property_party_view_name = 'property.vw_party_v3'
...
property_summary_cama_view_name='opa.vw_property_summary_cama_active_code_transition_dev'
...
property_summary_hybrid_trunc_view_name='opa.vw_property_summary_hybrid_trunc'
...
extract_property_summary_hybrid = GeopetlReadOperator(
    task_id='read_property_summary_hybrid',
    dag=pipeline,
    csv_path='{{ ti.xcom_pull("make_staging") }}/property_summary_hybrid.csv',
    db_conn_id='databridge2',
    db_table_name='opa.property_summary_hybrid',
    db_table_where='',
)
...
write_property_summary_hybrid_prod = GeopetlWriteOperator(
    task_id='write_property_summary_hybrid_prod',
    dag=pipeline,
    csv_path='{{ ti.xcom_pull("make_staging") }}/property_summary_hybrid.csv',
    db_conn_id='databridge-opa',
...
delete_temp_property_summary_hybrid = PythonOperator(
    task_id='delete_temp_property_summary_hybrid',
    dag=pipeline,
    python_callable=delete_temp_file,
    provide_context=True,
    templates_dict={'csv_path':'{{ ti.xcom_pull("make_staging") }}', 'filename':'property_summary_hybrid.csv',},
)
...
update_property_summary.set_upstream(write_address_servicearea_summary)
update_property_summary.set_upstream(update_property_assessments_for_summary_unfiltered)
update_property_summary.set_downstream(update_property_summary_hybrid)
...
write_property_summary_hybrid_prod.set_downstream(update_opa_properties_public)
...
write_property_summary_hybrid_prod.set_downstream(update_opa_properties_public_pde)
...
update_property_summary_hybrid.set_downstream(update_property_summary_hybrid_trunc)
update_property_summary_hybrid.set_downstream(extract_property_summary_hybrid)
# extract_property_summary_hybrid.set_downstream(write_property_summary_hybrid)
extract_property_summary_hybrid.set_downstream(write_property_summary_hybrid_prod)
# write_property_summary_hybrid.set_downstream(delete_temp_property_summary_hybrid)
write_property_summary_hybrid_prod.set_downstream(delete_temp_property_summary_hybrid)
delete_temp_property_summary_hybrid.set_downstream(cleanup)
