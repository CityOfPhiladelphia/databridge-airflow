...
pipeline = DAG('etl_parcel_editing_v1', schedule_interval='0 0 * * *', default_args=default_args)  # TODO: Look up how to schedule a DAG
dest_table_name='DOR_PARCEL_EDITS_ADDR_ANALYSIS'
update_dor_pwd_overlap_analysis_stmt = '''
BEGIN;
truncate table dor.dor_pwd_overlap_analysis4;
insert into dor.dor_pwd_overlap_analysis4 (select * from dor.vw_dor_pwd_overlap_analysis4);
COMMIT;
'''
...
def write_mssql(**kwargs):
    ...
    with open(test_file, 'r') as f:
        ...
        for row in reader:
            ...
            if i % buffer_size == 0:
                cursor.executemany(stmt, prep_rows)
                cursor.commit()
                prep_rows = []
...
extract_parcel_editing_records = MsSQLReadOperator(
    ...
    csv_path='{{ ti.xcom_pull("make_parcel_editing_staging1") }}/dor_parcels.csv',
    dag=pipeline,
...
update_parcel_editing_hash = PythonOperator(
    ...
    op_kwargs={'db_conn_id':'databridge2', 'table_schema':'dor', 'table_name': 'dor_parcel_edits_std1', 'hash_field': 'etl_hash'},
)
...
read_transformed_dor_parcel_editing_data = GeopetlReadOperator(
    ...
    csv_path='{{ ti.xcom_pull("make_parcel_editing_staging1") }}/vw_dor_parcel_edits_error_analysis.csv',
    dag=pipeline,
    db_conn_id='databridge2',
    db_table_name='dor.vw_dor_parcel_edits_error_analysis9',
)
...
write_transformed_dor_parcel_editing_data = PythonOperator(
    ...
    provide_context=True,
    templates_dict={'db_conn_id':'citygeo_editing', 'test_file':'{{ ti.xcom_pull("make_parcel_editing_staging1") }}/vw_dor_parcel_edits_error_analysis.csv', 'dest_table_name':dest_table_name},
)
...
read_transformed_dor_parcel_editing_data.set_upstream(update_dor_pwd_overlap_analysis)
