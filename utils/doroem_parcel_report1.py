...
def concatenate_dor_address(source_comps):
    ...
    address_low_fractional = source_comps['frac']
    ...
    try:
        ...
        if address_low_suffix_int == 2 and not address_low_fractional:
...
def check_address_comps(**kwargs):
    ...
    infile = file_path + '/' + kwargs['templates_dict']['infile_name'] + '.csv'
    ...
    {'house': c['house'], 'suf': c['suf'], 'frac': c['frac'], 'stex': c['stex'], 'stdir': c['stdir'], 'stnam': c['stnam'],
    ...
    .addfield('std_address_low_frac', lambda a: a['parsed_comps']['components']['address']['fractional']) \
    ...
    .addfield('change_frac', lambda a: 1 if a['no_address'] != 1 and str(standardize_nulls(a['frac'])) != str(
    standardize_nulls(a['std_address_low_frac'])) else None) \
